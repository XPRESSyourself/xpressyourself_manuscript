...
def rp_plot(
    ...
    for index, row_data in data_fig.iterrows():
        ...
        condBB = []
        condBB.append(condB_ribo[0]/condB_mrna[0])
        condBB.append(condB_ribo[1]/condB_mrna[1])

        condAA = []
        condAA.append(condA_ribo[0]/condA_mrna[0])
        condAA.append(condA_ribo[1]/condA_mrna[1])
        # Append p_value to dataframe
        try:
            statistic, p_value = stats.ttest_ind(condBB, condAA)
    ...
    print(data_fig[(data_fig['p_adj'] < 0.1) & (data_fig['ribo_fc'] > 2)].index.tolist())
    print(data_fig[(data_fig['p_adj'] < 0.1) & (data_fig['ribo_fc'] < 0.5)].index.tolist())

    print('Transcription targets')
    print(data_fig[(data_fig['p_adj'] < 0.1) & (data_fig['rna_fc'] > 2)].index.tolist())
    print(data_fig[(data_fig['p_adj'] < 0.1) & (data_fig['rna_fc'] < 0.5)].index.tolist())
    ...
    data_sig = data_fig[data_fig['p_adj'] < 0.1]
    ...
    #Plot data
    ax.scatter(rna_up, ribo_up, s=40,c='#be00be',alpha=0.8)
    ax.scatter(rna_down, ribo_down, s=40,c='green',alpha=0.8)
...
tm_data.loc[['ATF4','ATF5','PPP1R15A','DDIT3']]

tm_data.loc[['SLC1A1','HTRA1','SLC1A5']]

condB = []
condB.append(tm_data['ribo_tm_a'].loc['SLC1A1'] / tm_data['tm_a_hek'].loc['SLC1A1'])
condB.append(tm_data['ribo_tm_b'].loc['SLC1A1'] / tm_data['tm_b_hek'].loc['SLC1A1'])

condA = []
condA.append(tm_data['ribo_untr_a'].loc['SLC1A1'] / tm_data['untr_a_hek'].loc['SLC1A1'])
condA.append(tm_data['ribo_untr_b'].loc['SLC1A1'] / tm_data['untr_b_hek'].loc['SLC1A1'])

statistic, p_value = stats.ttest_ind(condB, condA, equal_var=True)
print(p_value)
...
tmisrib_data.loc['ATF4']['ribo_fc']
tmisrib_data.loc['ATF5']['ribo_fc']
tmisrib_data.loc['PPP1R15A']['ribo_fc']
tmisrib_data.loc['DDIT3']['ribo_fc']
