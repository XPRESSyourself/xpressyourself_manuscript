...
To provide the user with a example of the time each sub-module, we include a table breaking down the time it took for each individual sub-module to process this same data. However, this method repeats some index generation multiple times that the pipeline version will perform once, thus speeding up overall processing time for a pipeline run (Table \ref{tab:submodulebreakdown}).\\
...
\textbf{Metric} & \textbf{Command} & \textbf{Value} \\
  ...
  \hline
  Quantification & \texttt{count} &  \\
  \hline
  Differential Expression & \texttt{diffxpress} &  \\
  \hline
  Read Distributions & \texttt{readDistribution} &  \\
  \hline
  Metagene Analysis & \texttt{metagene} &  \\
  \hline
  Gene Coverage (1 gene) & \texttt{geneCoverage} &  \\
  \hline
