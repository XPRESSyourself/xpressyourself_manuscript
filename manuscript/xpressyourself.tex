...
Ensembl canonical transcripts are determined according to the Ensembl glossary definition of a canonical transcript \cite{ensembl_canon}. For cases where a tie exists between equal priority transcripts, the longest is chosen. When there are more than one transcript that tie for equal priority and longest length, the first listed record is retained. Exon or CDS lengths are calculated by taking the sum of each exon or CDS, not including intron or other space in the calculation. \par
...
If calculating meta-coverage over the CDS only, mapped meta-coordinates are corrected by subtracting the sum of the $5'$ UTR lengths for a transcript. Total length for a transcript is corrected by substracting the cumulative length of the $5'$ and $3'$ UTRs.
