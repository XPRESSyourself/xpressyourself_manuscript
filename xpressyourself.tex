...
\begin{enumerate}
  \item Expression Matrix: It is assumed the data matrix is \textit{i} * \textit{j} where \textit{i} (columns) are samples and \textit{j} (rows) are genes or other relative measurement points.
  \item Metagene Table: It is assumed the metagene table is a two column data matrix where column 0 is the sample ID (as specified in \textit{i} of the expression matrix) and column 1 is the sample group (for example, wild-type and treatment).
\end{enumerate}
...
\begin{enumerate}
  \item Probe uniquely maps to one gene and there are no redundant probes. In such a case, the probe ID is converted to the common gene name found in the probe index and no further manipulation is performed.
  \item Probe maps to multiple targets: In these cases, the user has two choices. They can either drop the probe from the analysis or leave as is and convert the probe ID to reflect all genes it maps to.
  \item Several probes map to a single gene: In these cases, the probe collapser will group these probes together and for each sample take the average measure of these probes. Probe ID will be converted to common gene name.
\end{enumerate}
...
\begin{itemize}
  ...
  \begin{enumerate}
    \item Compute the covariance of the two principle component arrays, \textit{x} and \textit{y}:

    \item Compute the eigenvalues and normalized eigenvectors of the covariance matrix:

    \item Compute the theta of the normalized eigenvectors:

    \item Compute the lambda of the eigenvalues:

    \item Plot the confidence intervals over the scatter plot: Using the patches library, the center point of the confidence interval is determined from the means of the \textit{x} and \textit{y} arrays. The width of the condfidence interval is calculated by \[ \textit{w} = lambda(\textit{x})\ x\ \textit{ci}\ x\ 2 \] where \textit{ci} is equal to the corresponding confidence level (i.e. 68\% = 1, 95\% = 2, 99\% = 3). The angle is set equal to \[ theta \].
  \end{enumerate}
...
In order to show the reliability of these tools, we sought to benchmark its output to other publically available datasets.
