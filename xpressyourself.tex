...
Based on these statistics, if HPC services weren't locally available for a user, one could use Amazon's AWS service. For the same run, storage cost would amount to around 14 USD on Amazon S3 storage and compute cost for a similar computational node for the given elapsed time would cost around 5 USD using Amazon EC2 On-Demand (however, significantly reduced rates are available if using Spot instances).
